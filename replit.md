...
- **Tower Judgement in Normal Chat Runs (2026-02-11)**: When `TOWER_LOOP_CHAT_MODE=true`, the normal `generateLeadsForChat` flow (in `server/supervisor.ts`) now calls `judgeArtefact` (same Tower client as Proof V2) immediately after persisting the `leads_list` artefact, instead of routing through `handleTowerVerdict` (agent-loop). It persists a `tower_judgement` artefact and emits `tower_call_started` → `tower_verdict` AFR events. If Tower is unreachable or throws, a `tower_judgement` artefact with `verdict: 'error'` is persisted (no "Judgement missing" limbo), the run is marked `stopped`, and the user is told results are available but validation was unavailable. The same error-resilient pattern was applied to `executeTowerLoopChat` (supervisor.ts) and `simulate-chat-task` (routes.ts). When `TOWER_LOOP_CHAT_MODE` is OFF, the original `handleTowerVerdict` path is preserved unchanged.
