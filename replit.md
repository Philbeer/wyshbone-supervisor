...
- **Deep research execution flow**: Two entry points share the same trigger→poll→artefact→events logic:
  - Chat tasks: `executeDeepResearchForChat` in `server/supervisor.ts` (private method on WyshboneSupervisor class).
  - Job handler: `runDeepResearchExecute` in `server/supervisor/jobs/handlers/deep-research-execute.ts` — wired to jobType `'deep_research'` in `server/supervisor/jobs.ts`. Runs standalone (no scheduler dependency). Reads `job.sourceRunId` as uiRunId, `job.clientRequestId`, `job.userId`, `job.payload.topic/prompt`. Includes standalone `postArtefact` and `bridgeRun` helpers.
  - Both: call UI `/api/tools/execute` with topic/prompt, poll `deep_research_runs` table by deepResearchRunId (5s interval, 5min max), POST `deep_research_result` artefact, emit gated events. Run bridge links uiRunId with deepResearchRunId.
...
- **Files**: `server/supervisor.ts`, `server/types/supervisor-chat.ts`, `server/routes.ts`, `server/supervisor/jobs.ts`, `server/supervisor/jobs/handlers/deep-research-execute.ts`