...
## Recent Changes
- **2026-02-13**: Fixed plan approval → plan executor runId mismatch. The `POST /api/plan/approve` endpoint now: (1) Accepts optional `run_id` and `client_request_id` from the request body. (2) Creates an `agent_run` record with the run ID (generated UUID if not provided). (3) Passes `jobId` and `clientRequestId` to `startPlanExecutionAsync`, ensuring the plan executor uses the correct run ID for all artefacts instead of generating its own `job_*` ID. (4) Returns `runId` and `clientRequestId` in the response. This ensures inline step_result and tower_judgement artefacts are stored under the same run ID that the UI queries, eliminating the need for backfill.
- **2026-02-13**: Enhanced diagnostic logging for inline Tower observation. Both `executeTowerLoopChat` and `plan-executor.ts` now log clear warnings when step_result creation fails or when Tower observation is skipped, including the affected runId for debugging.
- **2026-02-13**: Supervisor simplified to pure execution engine. Removed ~2,000 lines of dead/redundant code: (1) Deleted regex routing block from `processChatTask` (80+ lines of keyword classification that was never used for branching). (2) Deleted dead methods: `generateLeadsForChat` (~880 lines), `executeDeepResearchForChat` (~200 lines), `analyzeConversation` (~38 lines), `provideInsights` (~45 lines). (3) Replaced 861-line `simulate-chat-task` debug endpoint with a 45-line version that simply inserts a `supervisor_tasks` row, matching the production path. (4) Cleaned up `jobs.ts` deep_research redirect (removed unused `effectiveJobType` variable, streamlined logging). The supervisor now follows a clean flow: poll → receive task → `executeTowerLoopChat` → persist outputs → mark complete.
- **2026-02-13**: Supervisor-first routing: ALL chat messages now go directly through the supervisor's plan-based execution (`executeTowerLoopChat`). The `TOWER_LOOP_CHAT_MODE` feature flag has been removed — the plan path is always used. Deep research jobs from the UI are unconditionally redirected to supervisor_tasks (no keyword filtering), so the supervisor decides all tool routing.
- **2026-02-12**: Unconditional step-level observation-only Tower judgement across ALL execution paths. After every tool call completes, the system now unconditionally (no feature flags): (1) writes a `step_result` artefact to Supabase, (2) calls Tower to judge that `step_result`, (3) writes a `tower_judgement` artefact with `observation_only: true` in the payload. The verdict is logged (`[STEP_OBSERVATION]`) but never branched on — no retries, plan changes, or stopping. If the Tower call fails, the error is logged and the run continues unaffected.