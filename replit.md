...
## Recent Changes
- **2026-02-13**: Supervisor-first routing: ALL chat messages now go directly through the supervisor's plan-based execution (`executeTowerLoopChat`). The `TOWER_LOOP_CHAT_MODE` feature flag has been removed — the plan path is always used. Deep research jobs from the UI are unconditionally redirected to supervisor_tasks (no keyword filtering), so the supervisor decides all tool routing. The old switch-based routing in `processChatTask` (generate_leads, analyze_conversation, provide_insights, deep_research) has been replaced with a single plan-based execution path. A "google" keyword override in the routing logic forces SEARCH_PLACES for testing convenience.
- **2026-02-12**: Unconditional step-level observation-only Tower judgement across ALL execution paths. After every tool call completes, the system now unconditionally (no feature flags): (1) writes a `step_result` artefact to Supabase, (2) calls Tower to judge that `step_result`, (3) writes a `tower_judgement` artefact with `observation_only: true` in the payload. The verdict is logged (`[STEP_OBSERVATION]`) but never branched on — no retries, plan changes, or stopping. If the Tower call fails, the error is logged and the run continues unaffected. Coverage: `server/supervisor/plan-executor.ts` (plan executor steps), `server/supervisor.ts` (all 6 chat execution paths: `executeTowerLoopChat`, `generateLeadsForChat` x4 branches, `executeDeepResearchForChat`), `server/routes.ts` (`simulate-chat-task` endpoint), and `backfillTowerJudgements` poller (retroactive step_result + observation for runs created by external UI backend). The `isStepArtefactsEnabled()` gate has been removed — step_result writes are always unconditional. The existing conditional judgement flow (`shouldJudge`, `judgeStepResultSync`, reaction switch) and end-of-run judgement (safety net, `plan_result`) remain completely unchanged.