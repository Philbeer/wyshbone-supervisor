...
- ID Alignment Fix: `simulate-chat-task` now uses `uiRunId` (the `run_id` sent by the UI) as the canonical run ID for all artefact storage, agent run creation, and AFR logging. Previously, a separate `chatRunId` was generated via `generateJobId()`, causing a mismatch where Tower judgement artefacts were stored under an ID the UI couldn't query.
