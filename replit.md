...
- **Attribute Verification Gate (Website-First)**: Verifies hard constraints (e.g., "live music") after `SEARCH_PLACES` using a website-first strategy: visits `lead.website` from Google Places Details directly via `WEB_VISIT` (max_pages=3, page_hints=['home','events','whats-on']). Falls back to `WEB_SEARCH` only when no website is available. Includes a Brave 402/429 circuit breaker (`braveQuotaBreaker`) that trips on first quota error and skips remaining WEB_SEARCH calls with `unknown_reason=web_search_quota_exceeded`.
...
- **Attribute Verification Resilience**: Six-point fix for silent failures: (A) Brave empty-result detection with SEARCH_EMPTY error code, (B) 300ms pacing + 500ms backoff between WEB_SEARCH calls, (C) lead.website fallback when search returns empty, (D) keyword scan on page.title + page.text_clean (fixed field name mismatch from page.cleaned_text), (E) domain-root fallback for deep-path URLs, (F) awaited artefact writes. Debug with `ATTR_VERIFY_TRACE=1` env flag.
