...
- **Agent Loop v1 (Phase 1)**: Tower-governed replan loop for SEARCH_PLACES. After tool artefact creation, calls Tower judge-artefact endpoint (v1 verdict schema: ACCEPT/RETRY/CHANGE_PLAN/STOP). Maintains per-run state (RunState) with retry limit (max 1) and plan version limit (max 2). Posts run_summary artefact after every verdict. Debug endpoint at `/api/debug/simulate-tower-verdict` for testing without Tower changes.
