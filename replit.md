...
- **2026-02-13**: Verbatim goal preservation: `original_user_goal` captures the exact raw user message and is threaded through all artefacts (plan, step_result, leads_list, agent_run metadata). A separate `normalized_goal` is derived for internal use. The old synthetic "Find N pubs in Local for B2B outreach" string no longer overwrites the user's intent.
- **2026-02-13**: Plan v1 artefact: Before any tool execution, a `plan` artefact (type=plan, title="Plan v1") is created with `original_user_goal`, `normalized_goal`, `constraints` (count, business_type, location, prefix, tool preference), `assumptions`, and `steps` array with `step_id`, `tool`, `tool_args`, `expected_output`, and `post_processing`. This makes the plan explicit and visible in artefact history.
- **2026-02-13**: Improved intent parsing: Extracts prefix filter ("begin/start with X"), tool preference ("with google places search"), location, count, and business type from raw user message. Post-processing applies prefix filtering and count trimming after Google Places returns results. Google API key lookup now checks both `GOOGLE_PLACES_API_KEY` and `GOOGLE_MAPS_API_KEY`.