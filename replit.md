...
- **Demo Orchestrator (Feb 2026)**: `POST /api/demo/run/:scenario` with 3 deterministic scenarios: `clean_success` (5/5 PASS), `partial_delivery` (4/10 PARTIAL), `intelligent_stop` (0/20 STOP). Each creates a goal, builds canonical delivery_summary, writes beliefs, and updates goal status. `GET /api/demo/goals` and `GET /api/demo/goals/:goalId` for inspection.
