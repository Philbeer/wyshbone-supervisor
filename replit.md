...
- **Observability**: `[ARTEFACT_POST] runId=<...> clientRequestId=<...> status=<HTTP> hasArtefactId=<bool> artefactId=<...>` — one line per POST attempt. `[LEADS_ARTEFACT] uiRunId=<...> crid=<...> count=<N> posted=true/false status=<HTTP>` — one structured summary per leads artefact. `[DEEP_RESEARCH] uiRunId=<...> crid=<...> status=<status> reportChars=N sourcesCount=N posted=true/false artefactId=<...>` — one structured summary per deep research artefact. `[SUPERVISOR] ... uiRunId=<...> clientRequestId=<...>` for task processing.
- **Deep research provider**: `server/supervisor/research-provider.ts` — Perplexity-backed research provider behind a `DeepResearchProvider` interface. Uses `llama-3.1-sonar-large-128k-online` model. Requires `PERPLEXITY_API_KEY` env var. Returns `{ title, summary, report_markdown, sources: [{title, url}], status }`. Sources extracted from Perplexity's `citations` array.
- **Deep research execution flow**: Two entry points share the same research→artefact→events logic:
  ...
  - Job handler: `runDeepResearchExecute` in `server/supervisor/jobs/handlers/deep-research-execute.ts` — wired to jobType `'deep_research'` in `server/supervisor/jobs.ts`. Runs standalone (no scheduler dependency). Reads `job.sourceRunId` as uiRunId, `job.clientRequestId`, `job.userId`, `job.payload.topic/prompt`.
  - Both: call `createResearchProvider().research(topic, prompt)` to generate report via Perplexity API, POST `deep_research_result` artefact with `report_markdown` field, emit gated events. No UI tool delegation or polling.
...
- **simulate-chat-task**: Requires `run_id` and `client_request_id` in request body (returns 400 if missing). Supports `simulate_type` parameter: `'leads'` (default) mirrors SEARCH_PLACES path; `'deep_research'` runs real Perplexity research (not a stub) with canonical body, gated events, and `[DEEP_RESEARCH]` log.
