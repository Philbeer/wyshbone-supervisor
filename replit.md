...
- **Routing Hard Rule (2026-02-10)**: If a chat message contains pub/bar/venue keywords AND a location, the router forces `SEARCH_PLACES` (Google Places) as the tool, regardless of original `task_type`. A `tool_dispatch_decision` AFR event is emitted with `{ requested_count, parsed_location, chosen_tool, reason }`. `requested_count` is parsed from message and capped at 200.
