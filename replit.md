...
- **Gated events**: `artefact_created`, `run_completed`, `deep_research_completed` are ONLY emitted after POST returns 2xx and `artefactId` is parsed from the response. For SEARCH_PLACES, `run_completed` is ONLY emitted by the agent loop after a Tower ACCEPT verdict; all other SEARCH_PLACES paths emit `plan_execution_finished` instead.
- **Agent loop chat integration**: Chat flow (supervisor.ts) parses target_count from user messages (regex: `(\d+)\s+<businessType>`), creates `leads_list` artefact with `target_count`/`delivered_count`, calls `handleTowerVerdict`, and reacts to verdicts. `run_completed` only emitted after ACCEPT.
- **Tower judgement artefact**: Created after every Tower verdict (initial and re-judges in RETRY/CHANGE_PLAN paths) via `postTowerJudgementArtefact()`. Contains verdict, delivered, requested, gaps, confidence, rationale, plan_version.
- **Debug logging**: `[TOWER_CALL] url=<...> status=<HTTP>` for every Tower HTTP call. `[TOWER_VERDICT] verdict=<V> delivered=<N> requested=<N> confidence=<N>` for every verdict received. `[TOWER_TELEMETRY] tower_call_started/tower_call_finished runId=<...> verdict=<V> mode=<real|stub|simulated|no_url|error_fallback>` — one pair per Tower invocation, all paths.
- **Artefact ordering guarantee**: In agent-loop.ts, `postRunSummary` (artefact creation) ALWAYS executes BEFORE `emitRunCompleted`/`emitRunStopped` (status events). This ensures artefacts are visible before run status changes.
- **run_summary coverage**: run_summary artefact created on ALL SEARCH_PLACES terminal paths: ACCEPT, STOP, RETRY-exhaust, CHANGE_PLAN-exhaust, retry-fail, replan-fail, zero results (ZERO_RESULTS), error (ERROR), agent loop failure (AGENT_LOOP_ERROR).
- **[ARTEFACT_COUNT]**: `[ARTEFACT_COUNT] runId=<...> artefacts_written=<N> types=[<types>] verdict=<V>` — one log per SEARCH_PLACES chat run showing total artefacts created and final verdict.
