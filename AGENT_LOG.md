...
## Strategy Reviewer — LLM Override Layer for Mission Planner

**Date:** 2026-03-20
**Scope:** `server/supervisor/reloop/`
**Task:** Add an LLM review step that can override the mission planner's hardcoded strategy/tool_sequence decisions

---

### What Changed

A new module `strategy-reviewer.ts` was created and wired into the reloop entry point (`loop-skeleton.ts`). The mission planner continues to run and classify constraints as before; the strategy reviewer then gets a chance to confirm or override the chosen strategy before execution begins.

**New file: `server/supervisor/reloop/strategy-reviewer.ts`**

Exports a single async function `reviewStrategy()` that:

1. Receives the rules-based `MissionPlan`, the raw user input, and the `IntentNarrative`.
2. Builds a structured prompt summarising the constraints, the planner's decision, and available strategies (cheapest to most expensive).
3. Calls `gpt-4o-mini` with a 10-second timeout and `response_format: json_object`.
4. Validates the returned strategy against `VALID_STRATEGIES`; rejects unknown values and falls back to the original plan.
5. If the strategy changes, rebuilds `tool_sequence` and `steps` from `STRATEGY_TOOL_MAP` and updates `selection_reason` to record the override.
6. Writes a `diagnostic` artefact (via `createArtefact`) in both the override and confirm paths, capturing token counts, latency, and the LLM's reasoning.
7. Falls back silently (returns the original plan unchanged) on: missing `OPENAI_API_KEY`, timeout, HTTP error, or JSON parse failure.

**Modified file: `server/supervisor/reloop/loop-skeleton.ts`**

- Added import: `import { reviewStrategy } from './strategy-reviewer';`
- Added strategy review block immediately **before** the `missionContext` construction. The block calls `reviewStrategy()` and stores the result in `activePlan`.
- Changed `missionContext.plan` to reference `activePlan` instead of `plan`. All downstream code (executor, judge, gate) already reads from `missionContext`, so they naturally follow the (possibly overridden) strategy.
...
| File | Change |
|---|---|
| `server/supervisor/reloop/strategy-reviewer.ts` | **Created** — LLM strategy review with safe fallback |
| `server/supervisor/reloop/loop-skeleton.ts` | Added import + review block before `missionContext`; `plan` → `activePlan` in context |

---

### Decisions Made

- **`mission-planner.ts` untouched** — the planner still runs and provides the default strategy. The reviewer is purely additive.
- **`mission-executor.ts` untouched** — it reads `plan.strategy` and `plan.tool_sequence` from the context; if `activePlan` overrides those fields, the executor follows the new path with no code changes.
- **`composite` strategy excluded from `VALID_STRATEGIES`** — the prompt only offers the four concrete strategies; `composite` is a planner-internal concept and is not a valid LLM choice.
- **Artefact written on both paths** (confirm and override) to enable full observability of the reviewer's decisions in production.
- **All errors degrade gracefully** — the reviewer never throws; any failure returns `{ reviewed: false, overridden: false, plan: originalPlan }`.

---

### What's Next

- Monitor `diagnostic` artefacts tagged `Strategy review:` to measure how often the LLM overrides the planner and whether the overrides improve result quality.
- Consider adding a feature flag (`ENABLE_STRATEGY_REVIEW=false`) if the extra latency (~300–800 ms) becomes a concern in high-throughput paths.
- Evaluate whether the prompt's "Decision Rules" section should be expanded as more constraint types are added to the mission planner.
